...
G_d <- reshape2::melt(G)
# G_d$Var1 <- as.character(G_d$Var1)
# G_d$Var2 <- as.character(G_d$Var2)
# G_d$Var1[!G_d$Var1 %in% c("a", "b", "t")] <- ""
# G_d$Var2[!G_d$Var2 %in% c("a", "b", "t")] <- ""
...
G_d %>%
  ...
  theme(
    ...
    axis.text.x = element_text(color = ifelse(!G_d$Var1 %in% c("a", "b", "t"), "transparent", "black")),
...
* also note that SNR impacts the *scale* of each plot. I.e., although identical patterns are centered at zero (grey line) across the board, patterns with r = 0.5 (red line) are located increasingly nearer to zero with decreasing SNR (red line approaches grey line from L to R panels). In other words, the only absolutely meaningful value in these measures is zero. the ratio of measures is defined by the SNR.
    ...
    * what to use as the proper rescaling parameter? this question leads to complexities. 
        * e.g., say we use the estimated variability within each subject's RDM (e.g., across-cell SD). this estimate would be inflated because poorer SNRs are not only associated with smaller ratio scale, but *larger* variance. thus, subjects with lower SNR would be unfairly downweighted.
