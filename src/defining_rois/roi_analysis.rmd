...
means.1tr1knot <- abind(
  readRDS(here("out", "rsa", "means_shaefer400_baseline_Congruency_EVENTS_tentzero01210_censored.rds")),
  readRDS(here("out", "rsa", "means_shaefer400_proactive_Congruency_EVENTS_tentzero01210_censored.rds")),
  rev.along = 0
)
...
vanil.2tr1knot <- abind(
  ...
  -abind(
    readRDS(here("out", "rsa", "rmatrix_vanilla_eucl_shaefer400_baseline_Congruency_EVENTS_censored.rds")),
    readRDS(here("out", "rsa", "rmatrix_vanilla_eucl_shaefer400_proactive_Congruency_EVENTS_censored.rds")),
    rev.along = 0
  ),
  -abind(
    readRDS(here("out", "rsa", "rmatrix_vanilla_neuc_shaefer400_baseline_Congruency_EVENTS_censored.rds")),
    readRDS(here("out", "rsa", "rmatrix_vanilla_neuc_shaefer400_proactive_Congruency_EVENTS_censored.rds")),
    rev.along = 0
  ),
  rev.along = 0
)

names(dimnames(vanil.2tr1knot)) <- c(".row", ".col", "norma", "knot", "parcel", "subj", "session", "measure")
dimnames(vanil.2tr1knot)$session <- c("baseline", "proactive")
dimnames(vanil.2tr1knot)$measure <- c("corr", "neg.eucl", "neg.neuc")
...
# vanil.2tr1knot.d <- reshape2::melt(vanil.2tr1knot, value.name = "m") %>% mutate(glm = "2tr1knot")
# vanil.1tr1knot.d <- reshape2::melt(vanil.1tr1knot, value.name = "m") %>% mutate(glm = "1tr1knot")
# vanil.2tr1knot.d$tr <- as.numeric(gsub("knot", "", means.2tr1knot.d$knot)) * 2  ## knots to TRs
# vanil.1tr1knot.d$tr <- as.numeric(gsub("knot", "", means.1tr1knot.d$knot))
# vanil.d <- rbind(means.2tr1knot.d, means.1tr1knot.d)  ## bind different glms
# vanil.d$network <- ""  ## add networks
...
means.l.me <- means.d.1tr1knot %>% split(paste(.$tr, .$parcel, .$session, .$network, sep = "."))
...
coef.means.me %<>%  ## format
  ...
  cbind(reshape2::colsplit(.$model.id, "\\.", c("tr", "parcel", "session", "network")))
...
means.l.x <- means.d.1tr1knot[means.d.1tr1knot$pc == "PC50", ] %>% split(paste(.$tr, .$parcel, .$network, sep = "."))
...
coef.means.x %<>%  ## format
  bind_rows(.id = "model.id") %>%
  cbind(reshape2::colsplit(.$model.id, "\\.", c("tr", "parcel", "network")))
coef.means.x %<>%  ## adjust
  group_by(tr) %>%
  mutate(p.fdr = p.adjust(p.value, "fdr"))
...
coef.means.x %>%
  filter(term == "sessionproactive:trial.typeInCon") %>%
  ggplot(aes(tr, statistic)) +
    geom_beeswarm(
...
vanil.me.group %>%
  ...
  filter(glm == "2tr1knot", measure == "corr", norma == "raw") %>%
...
vanil.me.group %>%
  filter(glm == "2tr1knot", measure == "corr", norma == "prw", param == "congr") %>%
  ggplot(aes(tr, m)) +
  geom_beeswarm(
    aes(fill = network, alpha = p.fdr < 0.05, color = ifelse(p.fdr < 0.05, "black", "white")), 
    size = 1.5, shape = 21
  ...
  scale_fill_brewer(type = "qual", palette = 1) +
  # scale_x_continuous(limits = c(0, 9), breaks = seq(1, 8)) +
  labs(title = "incongruency model fit (pc50 items only)", x = "TR (post stimulus onset)", y = "contrast of z") +
  theme(legend.position = "none")
...
vanil.me.group %>%
  filter(glm == "1tr1knot", measure == "corr", norma == "raw", param == "congr") %>%
  group_by(session, parcel, network) %>%
  summarize(m = mean(m)) %>%
  tidyr::spread(session, m) %>%
  ggplot(aes(baseline, proactive)) +
  geom_point(aes(fill = network), size = 1.5, shape = 21, color = "white") +
  facet_grid(vars(network)) +
