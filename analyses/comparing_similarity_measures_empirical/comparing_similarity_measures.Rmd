...
All of these measures --- correlation, euclidean, standardized euclidean --- involve a __quadratic form__, a term that is multiplied to itself.
In correlation, for example, this form is the variance of each measure: $Var(x) = Cov(x, x) = E[(x - \bar{x})^2]$.
...
"Cross-validated" versions of these similarity measures all use the same trick to make them unbiased --- i.e., insensitive, in terms of expected value, to the error component.
This trick is to "swap out" one of these $x$ terms within the quadratic form with an independent "copy" of itself.
In cross-validated correlation, for example, the quadratic form for $Cov(x, x)$ is substituted with two independent copies of $x$, obtained from different scanning runs, $Cov(x_{(1)}, x_{(2)})$.
This swapping out creates an unbiased measure because the error terms cancel out (because of independence; see [Alink et al, 2015](https://www.biorxiv.org/content/10.1101/032391v2) for the algebra).
...
So, to correct for measurement error, the observed correlation values are scaled by their reliability.
...
For intuition, consider that the correlation is sensitive to the pattern "shape" (or, *angle* in vector space); scale (vector length) and mean differences (vector length along the unity line), however, are removed.
...
* then $\Sigma_1$ is inverted, e.g. $\mathbf{\Sigma}^{*-1}_1$, to perform the matrix division.
