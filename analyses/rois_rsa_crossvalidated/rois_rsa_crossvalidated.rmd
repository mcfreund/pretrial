...
cross %<>% filter(norma == "raw") %>% select(-norma)  ## raw values only (not prewhitened)
...
```{r, echo = TRUE}
# is.me.stroop <- cross$.row == "biasInCon" & cross$.col == "biasCon" | cross$.row == "PC50InCon" & cross$.col == "PC50Con"
# cross$congruency <- paste0(gsub("bias|PC50", "", cross$.row), "_", gsub("bias|PC50", "", cross$.col))
# cross$pc <- paste0(gsub("InCon|Con", "", cross$.row), "_", gsub("InCon|Con", "", cross$.col))
