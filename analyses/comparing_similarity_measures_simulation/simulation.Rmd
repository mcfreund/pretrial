...
Both of these are covariance matrices.
  ...
  - $\mathbf{\Sigma}:$ a $V$ (number of voxels) $\times \textit{V}$ covariance matrix.
    ...
    - To manipulate SNR, I multiplied **S** by a scalar value (while keeping **G** constant).
...
**2. Create B: the matrix that defines how the geometry is realized in a given set of voxels.**
  ...
  - To generate this I drew voxels from a multivariate distribution with mean vector $\mu_C$ and covariance matrix $\mathbf{G}$.
    - i.e., for voxel $v$, (a row of the **B** matrix), $\mathbf{b}_v \sim N_C(\mu_C, \mathbf{G})$, where $\mu_C$ is a vector of $C$ zeros.
...
**4. Create Y: raw voxel timecourses**
  ...
  - $\mathbf{Y} = \mathbf{XB}^T + \epsilon$, where each row of $\epsilon$ (each TR) is a draw $\sim N_V(\mu_V, \Sigma)$

**5. Estimate B via OLS**

  - $\hat{\mathbf{B}} = (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}\mathbf{Y}$

**6. Estimate G via RSA**
...
estimate_simil <- function(fit_1, fit_2) {
  ...
  cross_eucl_raw <- colSums(t(cmat %*% Bhat_1) * t(Bhat_2) %*% t(cmat)) / n_vox  ## euclidean (squared)
  cross_eucl_raw <- matrix(cross_eucl_raw, ncol = n_cond) / n_vox
  
  cross_eucl_prw <- colSums(t(cmat %*% Bhat_1 %*% W2) * t(Bhat_2) %*% t(cmat)) / n_vox  ## (mahalanobis) (squared)
  cross_eucl_prw <- matrix(cross_eucl_prw, ncol = n_cond) / n_vox
  
  cross_neuc_raw <- colSums(t(cmat %*% scale(Bhat_1)) * t(scale(Bhat_2)) %*% t(cmat)) / n_vox  ## norm. euclidean (squared)
  cross_neuc_raw <- matrix(cross_neuc_raw, ncol = n_cond) / n_vox
  
  cross_neuc_prw <- colSums(t(cmat %*% scale(Bhat_1) %*% W2) * t(scale(Bhat_2)) %*% t(cmat)) / n_vox  ## (norm. mahalanobis) (squared)
  cross_neuc_prw <- matrix(cross_neuc_prw, ncol = n_cond) / n_vox
...
To run the same plots / stats on these cross-valdated correlations, I set all NaN values equal to zero.
...
* prewhitening obviously increases sensivitivy across all noise thresholds
...
* the bias--variance tradeoff here is clear.
...
* "cross-validated" measures
  ...
  * the stripes of condition 't' dissappear: low bias
...
* also note that SNR impacts the *scale* of each plot. I.e., although identical patterns are centered at zero (grey line) across the board, patterns with r = 0.5 (red line) are located increasingly nearer to zero with decreasing SNR (red line approaches grey line from L to R panels). In other words, the only absolutely meaningful value in these measures is zero. the ratio of measures is defined by the SNR.
    * this may be imprtant to note: different subjects, brain regions may have different levels of SNR. Thus, some form of rescaling may be warranted prior to group analysis or comparing across regions.
...
4. if an unbiased correlation-like measure is desired -- i.e., invariance to scale and mean -- then standardized cross-validated mahalanobis may be suitible